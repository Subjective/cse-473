...
def constructBayesNet(gameState: hunters.GameState):
    ...
    observationValues = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(
        ...
        for eliminationVariable in eliminationOrder:
            currentFactors, joinedFactor = joinFactorsByVariable(
                currentFactors, eliminationVariable
            )
            if len(joinedFactor.unconditionedVariables()) > 1:
                currentFactors.append(eliminate(joinedFactor, eliminationVariable))
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            observationProbablity = self.getObservationProb(
                observation, pacmanPosition, position, jailPosition
            )
            updatedBeliefs[position] = observationProbablity * self.beliefs[position]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPosition in self.allPositions:
            ...
            for newPosition, probability in newPositionDistance.items():
                updatedBeliefs[newPosition] += oldProbability * probability
